...
def computeExtrinsic(img_path, mtx, dist, dX, dY):
    ...
    result = cv2.solvePnPRansac(obj_points.astype(np.float32), img_points.astype(np.float32), mtx, dist)
    #print(result)
    rvec = result[1]
    tvec = result[2]
    ...
    axis_img = (cv2.projectPoints(axis, rvec, tvec, mtx, dist)[0]).astype(int)
